...
export const GrowthSimulation: React.FC = () => {
  ...
  const calculateScenarios = useMemo((): GrowthScenario[] => {
      ...
      Object.entries(tickerGroups).forEach(([ticker, tickerGrants]) => {
        ...
        tickerGrants.forEach((grant) => {
          ...
          if (availableShares > 0) {
            ...
            expectedGross += taxResult.grossGain;
            expectedTax += taxResult.totalTax;
          }
        });
      });
  ...
  if (grants.length === 0) {
    return (
      <div className={`text-center py-8 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
        ...
        <p>Add grants first to run growth scenarios</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-800'} mb-2`}>Growth Simulation</h3>
        <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'} mb-2`}>
          Explore different growth scenarios and their impact on your portfolio value over time
        </p>
      ...
      <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border rounded-lg overflow-hidden`}>
        <h4 className={`text-md font-medium ${isDarkMode ? 'text-white border-gray-700' : 'text-gray-800 border-gray-200'} px-6 py-3 border-b`}>Growth Scenarios</h4>
      ...
      <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border rounded-lg p-6`}>
        <h4 className={`text-md font-medium ${isDarkMode ? 'text-white' : 'text-gray-800'} mb-4`}>Growth Scenario Visualization</h4>
        <div className="h-80">
